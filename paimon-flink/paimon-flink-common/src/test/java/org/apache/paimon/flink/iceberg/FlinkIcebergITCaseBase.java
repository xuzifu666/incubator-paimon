...
public abstract class FlinkIcebergITCaseBase extends AbstractTestBase {
    ...
    @ParameterizedTest
    @ValueSource(strings = {"orc", "parquet", "avro"})
    public void testNestedMultisetType(String format) throws Exception {
        String warehouse = getTempDirPath();
        TableEnvironment tEnv = tableEnvironmentBuilder().batchMode().parallelism(2).build();
        tEnv.executeSql(
                "CREATE CATALOG paimon WITH (\n"
        ...
        tEnv.executeSql(
                "CREATE TABLE paimon.`default`.T (\n"
                        + "  k INT,\n"
                        + "  v MAP<INT, MULTISET<ROW(f1 STRING, f2 INT)>>,\n"
                        + "  v2 BIGINT\n"
                        + ") WITH (\n"
                        + "  'metadata.iceberg.storage' = 'hadoop-catalog',\n"
                        + "  'file.format' = '"
        ...
        tEnv.executeSql(
                        "INSERT INTO paimon.`default`.T VALUES "
                                + "(1, MAP[10, MULTISET[ROW('apple', 100), ROW('banana', 101)], 20, MULTISET[ROW('cat', 102), ROW('dog', 103)]], 1000), "
                                + "(2, MAP[10, MULTISET[ROW('cherry', 200), ROW('pear', 201)], 20, MULTISET[ROW('tiger', 202), ROW('wolf', 203)]], 2000)")
                .await();
        ...
        tEnv.executeSql(
                        "INSERT INTO paimon.`default`.T VALUES "
                                + "(3, MAP[10, MULTISET[ROW('mango', 300), ROW('watermelon', 301)], 20, MULTISET[ROW('rabbit', 302), ROW('lion', 303)]], 3000)")
                .await();
        assertThat(
                        collect(
                                tEnv.executeSql(
    ...
    public void testCreateTags(String format) throws Exception {
        ...
        assertThat(
                        collect(
                                tEnv.executeSql(
                                        "SELECT v1, k, v2, pt FROM iceberg.`default`.T /*+ OPTIONS('tag'='tag2') */ ORDER BY pt, k")))
                .containsExactly(
                        Row.of(101, 10, "red", 1),
                        Row.of(110, 11, "banana", 1),
                        Row.of(121, 12, "green", 1),
